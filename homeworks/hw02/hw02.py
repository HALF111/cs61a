...
def pingpong_helper(i, n, num, incr):
    if i == n:
        return num
    else:
        if i % 8 == 0 or num_eights(i) >= 1:
            if incr:
                return pingpong_helper(i+1, n, num-1, not incr)
            else:
                return pingpong_helper(i+1, n, num+1, not incr)
        else:
            if incr:
                return pingpong_helper(i+1, n, num+1, incr)
            else:
                return pingpong_helper(i+1, n, num-1, incr)
...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    return pingpong_helper(1, n, 1, True)
...
def count_coins(total):
    ...
    def helper(total, coin_max_denomination):
        if total < 0:
            return 0
        elif total == 1:
            return 1
        elif coin_max_denomination == 1:
            return 1
        else:
            if total < coin_max_denomination:
                return helper(total, next_smallest_coin(coin_max_denomination))
            else:
                return helper(total-coin_max_denomination, coin_max_denomination) + \
                       helper(total, next_smallest_coin(coin_max_denomination))
